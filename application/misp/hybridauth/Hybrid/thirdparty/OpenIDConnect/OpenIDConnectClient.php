...
//        $response = $this->request($this->token_url, $params, $this->curl_authenticate_method);
        ...
        $response = $this->parseRequestResult($this->request($this->token_url, $params, $this->curl_authenticate_method));
    ...
    public function authenticated()
    {
        if ($this->access_token) {
            if ($this->token_info_url && $this->refresh_token && $this->id_token) {
                // check if this access token has expired, 
                $tokeninfo = $this->tokenInfo($this->id_token);
    ...
    // -- tokens
    public function tokenInfo($idToken = NULL)
    {
        if ($idToken) {
    ...
    {
        Hybrid_Logger::info("Enter OpenIDConnectClient::request( $url )");
        Hybrid_Logger::debug("OpenIDConnectClient::request(). dump request params: ", serialize($params));

        if ($type == "GET" && $params != FALSE) {
        ...
        Hybrid_Logger::debug("OpenIDConnectClient::request(). dump request info: ", serialize(curl_getinfo($ch)));
        Hybrid_Logger::debug("OpenIDConnectClient::request(). dump request result: ", serialize($response));
        Hybrid_Logger::debug("OpenIDConnectClient::request(). dump request header: ", serialize(curl_getinfo($ch, CURLINFO_HEADER_OUT)));
