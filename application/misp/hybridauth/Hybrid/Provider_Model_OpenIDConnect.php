 ...
 */
class Hybrid_Provider_Model_OpenIDConnect extends Hybrid_Provider_Model
{
    // default permissions 
    public $scope = "";
    ...
    function initialize()
    {
        ...
        require_once Hybrid_Auth::$config["path_libraries"] . "OpenIDConnect/OpenIDConnectClient.php";

        // create a new OAuth2 client instance
        $this->api = new OpenIDConnectCodeClient($this->config["keys"]["id"], $this->config["keys"]["secret"], $this->endpoint);
        ...
        if ($this->token("access_token")) {
            ...
            $this->api->token_type    = $this->token("token_type");
            $this->api->expires_in    = $this->token("expires_in");
            $this->api->expires_at    = $this->token("expires_at");
        }
    ...
    function loginFinish()
    {
        ...
        $this->token("expires_in", $this->api->expires_in);
        $this->token("expires_at", $this->api->expires_at);
    ...
    function refreshToken()
    {
        ...
        if ($this->api->access_token) {
            ...
            // have to refresh?
            if ($this->api->refresh_token && $this->api->expires_at) {

                // expired?
                if ($this->api->expires_at <= time()) {
                    ...
                    if (isset($response->token_type))
                        $this->api->token_type = $response->token_type;


                    if (isset($response->expires_in)) {
                        $this->api->expires_in = $response->expires_in;

                        // even given by some idp, we should calculate this
                        $this->api->expires_at = time() + $response->expires_in;
                    }
                }
            }
