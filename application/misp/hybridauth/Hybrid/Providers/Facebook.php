...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
	...
	*/
    ...
    protected function _post($uri, $data)
    {
        ...
        $header  = $this->_parseHeader($http_response_header);
        if ($this->_isHttpFail($header['Status'])) {
    ...
    protected function _isHttpFail($status)
    {
        return (bool) (empty($status) || ($status >= 400));
    }
    
    protected function _requestAppAccessToken()
    {
        $body                   = array();
        $body['client_id']      = $this->api->getAppId();
        $body['client_secret']  = $this->api->getAppSecret();
    ...
    protected function _requestDebugToken()
    {
        // リクエストボディを作成
        $body                  = array();
        $body['input_token']    = $this->api->getAccessToken();
        $body['access_token']   = $this->_requestAppAccessToken();

        // エンドポイントURL設定
        $endpointUrl = 'https://graph.facebook.com/debug_token?' . http_build_query($body, null, '&');

        // リクエストの送信とレスポンスの取得
        return json_decode(file_get_contents($endpointUrl), TRUE);
    }
    
    protected function _isValidAccessToken()
    {
        $response = $this->_requestDebugToken();        
        $data = $response['data'];
        
        // 不正なアクセストークンかどうかをチェック
        if (!isset($data['is_valid'])) {
            if (isset($data['error']) && isset($data['error']['message'])) {
                throw new Common_Exception_Oidc_InvalidToken($data['error']['message']);
            } else {
                throw new Common_Exception_Oidc_InvalidToken('アクセストークンが不正です');
            }
        }
        
        // アクセストークンの発行元を確認
        if (!isset($data['app_id']) || (string)$data['app_id'] !== (string)$this->api->getAppId()) {
            throw new Common_Exception_Oidc_InvalidToken('アクセストークンのAppIdが、発行元のAppIdと一致しませんでした');
        }
        
        // ユーザーIDを確認
        if (!isset($data['user_id']) || (string)$data['user_id'] !== (string)$this->api->getUser()) {
            throw new Common_Exception_Oidc_InvalidToken('アクセストークンのユーザIDが、リクエストしているユーザのIDと一致しませんでした');
        }
