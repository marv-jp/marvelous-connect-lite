...
class Hybrid_Auth
{
    ...
    public static function initialize($config)
    {
        ...
        require_once $config["path_base"] . "Provider_Model_OpenIDConnect.php";
    ...
    public static function redirect($url, $mode = "PHP")
    {
        ...
        if ($mode == "PHP") {
            $userAgent = new Common_Http_UserAgent();
            if ($userAgent->isIe()) {
                // このヘッダーについては http://jira.maql.co.jp/browse/STAN-342 参照
                header("P3P: CP='IDC DSP LAW CUR ADM DEV PSA PSD IVA IVD CON HIS OUR IND UNI COM NAV INT CNT STA'");
            }
            header("Location: $url");
        } elseif ($mode == "JS") {
            echo '<html>';
