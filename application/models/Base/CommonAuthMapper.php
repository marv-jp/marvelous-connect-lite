...
/**
 * Application_Model_Base_CommonAuthMapperクラス
 *
 * 自動生成クラス
 *
 * @category Zend
 ...
 * @subpackage Base
 */
abstract class Application_Model_Base_CommonAuthMapper extends Application_Model_BaseMapper
{

    /**
     * @var Zend_Db_Table Zend_Db_Table
     */
    protected $_modelName = 'Application_Model_CommonAuth';

    /**
     * TableDataを返す。
     *
     * @return Application_Model_DbTable_CommonAuth Application_Model_DbTable_CommonAuthのオブジェクト
     */
    public function getDbTable()
    {
        if (null === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_CommonAuth');
        }
     ...
     * モデルオブジェクトのデータを返す。
     *
     * @return array Application_Model_CommonAuthのオブジェクトから取り出したデータ
     */
    protected function _dataSet(Application_Model_CommonAuth $commonAuth)
    {
        return array(
        'realm' => $commonAuth->getRealm(),
        'username' => $commonAuth->getUsername(),
        'display_name' => $commonAuth->getDisplayName(),
        'password' => $commonAuth->getPassword(),
        'role' => $commonAuth->getRole(),
        );
    }
     ...
     *
     * @param Application_Model_CommonAuth $commonAuth Application_Model_CommonAuthのオブジェクト
     * @return mixed The primary key of the row inserted.
     */
    public function insertWithDefaultValues(Application_Model_CommonAuth $commonAuth)
    {
        $data = $this->_dataSet($commonAuth);
     ...
     * @return Application_Model_Base_CommonAuthMapper Application_Model_Base_CommonAuthMapperのオブジェクト
     */
     ...
     * @return array Application_Model_CommonAuthのオブジェクト配列
     */
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function insert(Application_Model_CommonAuth $commonAuth)
    {
        return parent::insert($commonAuth);
    }
     ...
     * 結果が1件の場合は、Application_Model_CommonAuthのオブジェクトを返す。<br>
     * 結果が複数件の場合は、Application_Model_CommonAuthの配列を返す。<br>
     * <br>
     * 使用方法<br>
     * <pre>
     * // 単一の行を探す。
     ...
     * @return Application_Model_CommonAuth|array メソッドの説明を参照
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function replace(Application_Model_CommonAuth $commonAuth)
    {
        return parent::replace($commonAuth);
    }
