...
/**
 * Application_Model_Base_SessionMapperクラス
 *
 * 自動生成クラス
 *
 * @category Zend
 ...
 * @subpackage Base
 */
abstract class Application_Model_Base_SessionMapper extends Application_Model_BaseMapper
{

    /**
     * @var Zend_Db_Table Zend_Db_Table
     */
    protected $_modelName = 'Application_Model_Session';

    /**
     * TableDataを返す。
     *
     * @return Application_Model_DbTable_Session Application_Model_DbTable_Sessionのオブジェクト
     */
     ...
     * モデルオブジェクトのデータを返す。
     *
     * @return array Application_Model_Sessionのオブジェクトから取り出したデータ
     */
    protected function _dataSet(Application_Model_Session $session)
    {
        return array(
        'id' => $session->getId(),
        'modified' => $session->getModified(),
     ...
     *
     * @param Application_Model_Session $session Application_Model_Sessionのオブジェクト
     * @return mixed The primary key of the row inserted.
     */
    public function insertWithDefaultValues(Application_Model_Session $session)
    {
        $data = $this->_dataSet($session);
     ...
     * @return Application_Model_Base_SessionMapper Application_Model_Base_SessionMapperのオブジェクト
     */
     ...
     * @return array Application_Model_Sessionのオブジェクト配列
     */
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function insert(Application_Model_Session $session)
    {
        return parent::insert($session);
    }
     ...
     * 結果が1件の場合は、Application_Model_Sessionのオブジェクトを返す。<br>
     * 結果が複数件の場合は、Application_Model_Sessionの配列を返す。<br>
     * <br>
     * 使用方法<br>
     * <pre>
     * // 単一の行を探す。
     ...
     * @return Application_Model_Session|array メソッドの説明を参照
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function replace(Application_Model_Session $session)
    {
        return parent::replace($session);
    }
