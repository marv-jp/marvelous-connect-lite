...
/**
 * Application_Model_Base_PlatformUserMapperクラス
 *
 * 自動生成クラス
 *
 * @category Zend
 ...
 * @subpackage Base
 */
abstract class Application_Model_Base_PlatformUserMapper extends Application_Model_BaseMapper
{

    /**
     * @var Zend_Db_Table Zend_Db_Table
     */
    protected $_modelName = 'Application_Model_PlatformUser';

    /**
     * TableDataを返す。
     *
     * @return Application_Model_DbTable_PlatformUser Application_Model_DbTable_PlatformUserのオブジェクト
     */
    public function getDbTable()
    {
        if (null === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_PlatformUser');
        }
     ...
     * モデルオブジェクトのデータを返す。
     *
     * @return array Application_Model_PlatformUserのオブジェクトから取り出したデータ
     */
    protected function _dataSet(Application_Model_PlatformUser $platformUser)
    {
        return array(
        'platform_user_id' => $platformUser->getPlatformUserId(),
        'platform_id' => $platformUser->getPlatformId(),
        'platform_user_name' => $platformUser->getPlatformUserName(),
        'platform_user_display_name' => $platformUser->getPlatformUserDisplayName(),
        'email_address' => $platformUser->getEmailAddress(),
        'access_token' => $platformUser->getAccessToken(),
        'id_token' => $platformUser->getIdToken(),
        'status' => $platformUser->getStatus(),
        'created_date' => $platformUser->getCreatedDate(),
        'updated_date' => $platformUser->getUpdatedDate(),
        'deleted_date' => $platformUser->getDeletedDate(),
        );
    }
     ...
     *
     * @param Application_Model_PlatformUser $platformUser Application_Model_PlatformUserのオブジェクト
     * @return mixed The primary key of the row inserted.
     */
    public function insertWithDefaultValues(Application_Model_PlatformUser $platformUser)
    {
        $data = $this->_dataSet($platformUser);
     ...
     * @return Application_Model_Base_PlatformUserMapper Application_Model_Base_PlatformUserMapperのオブジェクト
     */
     ...
     * @return array Application_Model_PlatformUserのオブジェクト配列
     */
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function insert(Application_Model_PlatformUser $platformUser)
    {
        return parent::insert($platformUser);
    }
     ...
     * 結果が1件の場合は、Application_Model_PlatformUserのオブジェクトを返す。<br>
     * 結果が複数件の場合は、Application_Model_PlatformUserの配列を返す。<br>
     * <br>
     * 使用方法<br>
     * <pre>
     * // 単一の行を探す。
     ...
     * @return Application_Model_PlatformUser|array メソッドの説明を参照
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function replace(Application_Model_PlatformUser $platformUser)
    {
        return parent::replace($platformUser);
    }
