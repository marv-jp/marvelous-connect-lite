...
/**
 * Application_Model_Base_PlatformMapperクラス
 *
 * 自動生成クラス
 *
 * @category Zend
 ...
 * @subpackage Base
 */
abstract class Application_Model_Base_PlatformMapper extends Application_Model_BaseMapper
{

    /**
     * @var Zend_Db_Table Zend_Db_Table
     */
    protected $_modelName = 'Application_Model_Platform';

    /**
     * TableDataを返す。
     *
     * @return Application_Model_DbTable_Platform Application_Model_DbTable_Platformのオブジェクト
     */
    public function getDbTable()
    {
        if (null === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_Platform');
        }
     ...
     * モデルオブジェクトのデータを返す。
     *
     * @return array Application_Model_Platformのオブジェクトから取り出したデータ
     */
    protected function _dataSet(Application_Model_Platform $platform)
    {
        return array(
        'platform_id' => $platform->getPlatformId(),
        'platform_name' => $platform->getPlatformName(),
        'platform_domain' => $platform->getPlatformDomain(),
        'platform_type' => $platform->getPlatformType(),
        'sort_order' => $platform->getSortOrder(),
        'created_date' => $platform->getCreatedDate(),
        'updated_date' => $platform->getUpdatedDate(),
        'deleted_date' => $platform->getDeletedDate(),
        );
    }
     ...
     *
     * @param Application_Model_Platform $platform Application_Model_Platformのオブジェクト
     * @return mixed The primary key of the row inserted.
     */
    public function insertWithDefaultValues(Application_Model_Platform $platform)
    {
        $data = $this->_dataSet($platform);
     ...
     * @return Application_Model_Base_PlatformMapper Application_Model_Base_PlatformMapperのオブジェクト
     */
     ...
     * @return array Application_Model_Platformのオブジェクト配列
     */
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function insert(Application_Model_Platform $platform)
    {
     ...
     * 結果が1件の場合は、Application_Model_Platformのオブジェクトを返す。<br>
     * 結果が複数件の場合は、Application_Model_Platformの配列を返す。<br>
     * <br>
     * 使用方法<br>
     * <pre>
     * // 単一の行を探す。
     ...
     * @return Application_Model_Platform|array メソッドの説明を参照
     ...
     * @return mixed The primary key of the row inserted.
     */
    public function replace(Application_Model_Platform $platform)
    {
        return parent::replace($platform);
    }
